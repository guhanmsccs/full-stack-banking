...
export default function Withdraw() {
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      setLoading(true);
      try {
        const response = await axios.post("http://localhost:5000/user-action", { token });
  ...
  const handleTransactions = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post("http://localhost:5000/user-action", { token, amount: amountValue });
